...
export function DedicatedVendorProductForm({ initialData }: { initialData?: any }) {
  ...
  const [formData, setFormData] = useState<ProductFormData>(() => {
    if (initialData) {
      return {
        ...
        deadWeight: initialData.deadWeight || 0,
  ...
  const handleInputChange = (field: keyof ProductFormData, value: any) => {
    setFormData(prev => {
      ...
      // Auto-calc final sale price when sale price or commission changes
      if (['salePrice', 'commissionAmount'].includes(field)) {
        // Get the base sale price (what user enters)
        let baseSalePrice = 0;
        if (field === 'salePrice') {
          baseSalePrice = Number(next.salePrice) || 0;
        } else {
          // If commission is changing, get the original sale price before commission was added
          const currentDisplayedPrice = Number(formData.salePrice) || 0;
          const oldCommission = Number(formData.commissionAmount) || 0;
          baseSalePrice = currentDisplayedPrice - oldCommission;
        }
        
        const commission = Number(next.commissionAmount) || 0;
        
        // Calculate final sale price: Sale Price + Commission
        const calculated = baseSalePrice + commission;
        next.salePrice = Number(calculated.toFixed(2));
        
        console.log('Commission Calculation Fixed:', {
          field: field,
          baseSalePrice: baseSalePrice,
          commissionAmount: commission,
          calculatedFinalPrice: calculated,
          finalSalePriceField: next.salePrice,
          explanation: commission === 0 ? 'Commission is 0, final sale price equals base sale price' : 'Commission added to sale price'
        });
      }
      return next;
    });
  ...
  const handleNumberChange = (
    ...
    if (v === '') {
      // Use handleInputChange to trigger auto-calculation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Form data id field:', (formData as any).id);
      console.log('Form data id type:', typeof (formData as any).id);
  ...
  return (
    <div className="w-full max-w-none scrollbar-hide overflow-auto">
      <form onSubmit={handleSubmit} className="w-full space-y-8 scrollbar-hide">
          {/* Basic Information */}
          ...
          <Card className="w-full border-0 shadow-lg">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <Label htmlFor="price">Cut Price (Reference only) *</Label>
                ...
                <div className="space-y-3">
                <Label htmlFor="salePrice">Sale Price (Base Price + Commission)</Label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 transform -translate-y-1/2 text-muted-foreground">₹</span>
                    <Input
                      id="salePrice"
                ...
                <p className="text-xs text-muted-foreground">
                  <strong>Calculation:</strong> Sale Price + ₹{formData.commissionAmount || 0} (Commission) = ₹{formData.salePrice || 0} (Final Sale Price)
                </p>
                </div>
          ...
          <Card className="w-full border-0 shadow-lg">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-8">
                <div className="space-y-4">
                  <Label className="text-lg font-semibold">Primary Material (Multi-select)</Label>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {['None', 'Wood (Teak)', 'Wood (Oak)', 'Wood (Engineered)', 'Metal', 'Glass', 'Plastic', 'Rattan', 'Fabric', 'Marble', 'Granite', 'Ceramic', 'Bamboo'].map((materialOption) => (
                      <div key={materialOption} className="flex items-center space-x-2 p-2 rounded-md border hover:bg-accent transition-colors">
                        <input
                          type="checkbox"
                          ...
                          className="rounded"
                        />
                        <Label htmlFor={`material-${materialOption.toLowerCase().replace(/[^a-z0-9]/g, '-')}`} className="text-sm font-medium cursor-pointer">
                          {materialOption}
                        </Label>
                      </div>
                    ))}
                ...
                <div className="space-y-4">
                  <Label className="text-lg font-semibold">Finish (Multi-select)</Label>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {['None', 'Matte', 'Glossy', 'Textured', 'Polished', 'Painted', 'Natural', 'Distressed', 'Brushed', 'Anodized', 'Powder Coated'].map((finishOption) => (
                      <div key={finishOption} className="flex items-center space-x-2 p-2 rounded-md border hover:bg-accent transition-colors">
                        <input
                          type="checkbox"
                          ...
                          className="rounded"
                        />
                        <Label htmlFor={`finish-${finishOption.toLowerCase().replace(/\s+/g, '-')}`} className="text-sm font-medium cursor-pointer">
                          {finishOption}
                        </Label>
                      </div>
                    ))}
              ...
              <div className="space-y-4">
                <Label className="text-lg font-semibold">Upholstery Material (Multi-select)</Label>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {['None', 'Cotton', 'Linen', 'Velvet', 'Leatherette', 'Jute', 'Wool', 'Silk', 'Polyester', 'Microfiber', 'Suede', 'Canvas'].map((upholsteryOption) => (
                    <div key={upholsteryOption} className="flex items-center space-x-2 p-2 rounded-md border hover:bg-accent transition-colors">
                      <input
                        type="checkbox"
                        ...
                        className="rounded"
                      />
                      <Label htmlFor={`upholstery-${upholsteryOption.toLowerCase()}`} className="text-sm font-medium cursor-pointer">
                        {upholsteryOption}
                      </Label>
                    </div>
                  ))}
          ...
          <Card className="w-full border-0 shadow-lg">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  <Label htmlFor="deadWeight">Dead Weight (in Kg)</Label>
                  <Input
                    id="deadWeight"
                    type="number"
                    step="0.01"
                    value={Number.isFinite(formData.deadWeight) ? formData.deadWeight : ''}
                    onChange={handleNumberChange('deadWeight', parseFloat)}
                    placeholder="e.g., 1.8"
                    className="h-12"
                  />
                </div>
